...
class BTreeIndex {
  ...
  **/
  ...
  const void insertInteger(RIDKeyPair<int> entry, bool leaf, PageId pageNo, PageId updatedId, int newValInt);
  ...
  const void insertNonLeafInteger(RIDKeyPair<int> entry, PageId pageNo, bool leaf, int newNodeVal, PageId& updatedId);
  const void insertLeafInteger(RIDKeyPair<int> entry, bool leaf, PageId pageNo, PageId& updatedId, int & nodeValue);
  const void splitLeaf(int position, RIDKeyPair<int> entry, LeafNodeInt* currNode, PageId& updatedId, int & leafValue);
  const void splitNode(int position, NonLeafNodeInt* currNode, PageId& updatedId, int & splitVal, PageId pageNo);
};
